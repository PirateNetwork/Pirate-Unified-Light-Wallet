...
String friendlySwapError(Object? error) {
  if (error == null) return 'Something went wrong with the swap.';
  ...
  if (text.contains('timed out waiting for your ltc deposit') ||
    ...
    return "We didn't detect your deposit in time. If you already sent "
        'it, it may still be confirming - check the funding balance and '
        'try again, or refund it from there.';
  }

  if (text.contains('timed out waiting for arrr')) {
    return "Your ARRR hasn't reached the swap engine yet. It may still be "
        'confirming on-chain. You can try again shortly.';
  }

  if (text.contains('timed out waiting for the atomic swap')) {
    return 'The swap is taking longer than expected to settle. It may still '
        'complete - check your open orders and balances before retrying.';
  }

  if (text.contains('notsufficientbalance') ||
      text.contains('insufficient') ||
      text.contains('not enough')) {
    return "There isn't enough balance to complete this swap, including "
        'network fees. Try a smaller amount.';
  }
  ...
  if (text.contains('no asks') ||
    ...
    return "There aren't enough orders on the book to fill this trade right "
        'now. Try a smaller amount or check back shortly.';
  }

  if (text.contains('not running') ||
      text.contains('failed to start') ||
      text.contains('rpc password')) {
    return "The swap engine isn't ready yet. Give it a moment to connect and "
        'try again.';
  }
