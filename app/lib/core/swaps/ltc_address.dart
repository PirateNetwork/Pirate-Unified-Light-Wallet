...
LtcAddressCheck checkLtcAddress(String raw) {
  ...
  if (address.isEmpty) {
    return LtcAddressCheck.invalid('Enter the LTC address to receive funds.');
  }
  ...
  if (lower.startsWith('ltc1')) {
    if (address.length < 26 || address.length > 90) {
      return LtcAddressCheck.invalid('This LTC address looks incomplete.');
    }
    ...
    if (hasInvalidChar) {
      return LtcAddressCheck.invalid(
        'This LTC address contains invalid characters.',
      );
    }
  ...
  return LtcAddressCheck.invalid(
    'Enter a valid Litecoin address (starts with ltc1, L, M, or 3).',
  );
}
