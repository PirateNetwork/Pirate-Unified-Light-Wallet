...
use pirate_sync_lightd::{SyncEngine, SyncMode, SyncConfig};
...
async fn run_sync_benchmark(
    ...
    // For now, measure theoretical throughput
...
fn get_memory_usage_mb() -> usize {
    // In production, use jemalloc stats or /proc/self/status
...
async fn bench_compact_sync_10k_blocks() -> Result<(), Box<dyn std::error::Error>> {
    let metrics = run_sync_benchmark(10_000, SyncMode::Compact).await?;
    ...
    assert!(metrics.blocks_per_second > 100.0, "Should sync >100 blocks/sec");
...
async fn bench_deep_sync_10k_blocks() -> Result<(), Box<dyn std::error::Error>> {
    let metrics = run_sync_benchmark(10_000, SyncMode::Deep).await?;
    ...
    assert!(metrics.blocks_per_second > 50.0, "Should sync >50 blocks/sec in deep mode");
...
async fn bench_checkpoint_creation_overhead() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Average time: {} µs", avg_checkpoint_time);
    
    // Checkpoint creation should be <1ms
    assert!(avg_checkpoint_time < 1000, "Checkpoint creation should be <1ms");
...
async fn bench_sync_progress_calculation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Progress calculation should be negligible (<1µs)
    assert!(calcs_per_second > 1_000_000.0, "Progress calculation should be >1M/sec");
...
async fn bench_rollback_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tx = conn.unchecked_transaction()?;
    for i in 0..10_000 {
        tx.execute("INSERT INTO blocks (height, data) VALUES (?1, ?2)", (i, vec![0u8; 100]))?;
        if i % 1000 == 0 {
...
async fn bench_memory_usage_during_sync() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let memory_increase = peak_mem - start_mem;
