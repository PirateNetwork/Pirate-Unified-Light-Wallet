...
async fn test_sync_timeout_and_resume() {
    ...
    let sync_result = timeout(
        ...
        engine.sync_range(4_000_000, 4_100_000),
    )
    .await;
...
async fn test_checkpoint_creation() {
    ...
    println!(
        "Last checkpoint: {:?}",
        final_progress.last_checkpoint()
    );
...
async fn test_graceful_shutdown() {
    ...
    let sync_handle = tokio::spawn(async move {
        engine.sync_range(4_000_000, 4_100_000).await
    });
...
async fn test_network_interruption_recovery() {
    ...
    let result = timeout(
        ...
        engine.sync_range(4_000_000, 4_010_000),
    )
    .await;
...
async fn test_multiple_interruptions() {
    ...
    let mut current_height = 4_000_000;
    let target_height = 4_010_000;
    ...
    for i in 0..interrupt_count {
        ...
        let result = engine.sync_range(current_height, next_height).await;
        assert!(result.is_ok(), "Sync segment {} failed", i);

        println!(
            "Segment {}: {} -> {} ✅",
            i, current_height, next_height
        );
