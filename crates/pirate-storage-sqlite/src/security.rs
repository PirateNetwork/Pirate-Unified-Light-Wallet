...
impl MasterKey {
    ...
    fn decrypt_aes_gcm(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if algorithm != 0 {
            return Err(Error::Encryption(format!(
                "Algorithm mismatch: expected AES-GCM (0), got {}",
                algorithm
            )));
        }
    ...
    fn decrypt_chacha20(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if algorithm != 1 {
            return Err(Error::Encryption(format!(
                "Algorithm mismatch: expected ChaCha20-Poly1305 (1), got {}",
                algorithm
            )));
        }
...
impl AppPassphrase {
    ...
    pub fn verify(&self, passphrase: &str) -> Result<bool> {
        let parsed_hash =
            PasswordHash::new(&self.hash).map_err(|e| Error::Encryption(e.to_string()))?;
...
impl SealedKey {
    ...
    pub fn new(encrypted_key: Vec<u8>, key_id: String, algorithm: EncryptionAlgorithm) -> Self {
        Self {
            encrypted_key,
            key_id,
            algorithm,
...
mod tests {
    ...
    fn test_passphrase_strength_evaluation() {
        ...
        assert_eq!(
            AppPassphrase::evaluate_strength("1234567"),
            PassphraseStrength::Weak
        );
