...
use pirate_storage_sqlite::keystore::{
    BiometricConfig, BiometricManager, BiometricState, KeystoreCapabilities, KeystoreResult,
    MockKeystore, Platform, PlatformKeystore,
};
use pirate_storage_sqlite::screenshot_guard::{
    ProtectionReason, ProtectionState, ScreenshotGuard, ScreenshotProtectionStatus,
};
use pirate_storage_sqlite::secure_clipboard::{
    ClipboardDataType, ClipboardPlatform, MockClipboard, SecureClipboard,
    DEFAULT_CLEAR_TIMEOUT_SECS, IVK_CLEAR_TIMEOUT_SECS, SEED_CLEAR_TIMEOUT_SECS,
};
use pirate_storage_sqlite::security::{
    generate_salt, hash_sha256, AppPassphrase, EncryptionAlgorithm, MasterKey, PanicPin, SealedKey,
};
use pirate_storage_sqlite::seed_export::{ExportFlowState, SeedExportManager, SeedExportResult};
...
fn test_passphrase_hash_uses_argon2id() {
    ...
    assert!(
        hash_string.starts_with("$argon2id$"),
        "Hash must use Argon2id: {}",
        hash_string
    );
    ...
    assert!(
        hash_string.contains("m=65536"),
        "Hash must use 64 MiB memory: {}",
        hash_string
    );

    // Should contain time parameter t=3
    assert!(
        hash_string.contains("t=3"),
        "Hash must use 3 iterations: {}",
        hash_string
    );

    // Should contain parallelism parameter p=4
    assert!(
        hash_string.contains("p=4"),
        "Hash must use 4 lanes: {}",
        hash_string
    );
}
...
fn test_passphrase_strength_evaluation() {
    ...
    // Weak (< 8 chars)
    assert_eq!(
        AppPassphrase::evaluate_strength("short"),
        PassphraseStrength::Weak
    );

    // Fair (8-11 chars)
    assert_eq!(
        AppPassphrase::evaluate_strength("password12"),
        PassphraseStrength::Fair
    );

    // Good (12-15 chars or lacks variety)
    assert_eq!(
        AppPassphrase::evaluate_strength("MyPassword123"),
        PassphraseStrength::Good
    );

    // Strong (16+ chars with variety)
    assert_eq!(
        AppPassphrase::evaluate_strength("MySecurePass123!@#"),
...
fn test_clipboard_timeout_values() {
    ...
    assert_eq!(
        SEED_CLEAR_TIMEOUT_SECS, 10,
        "Seed phrase must auto-clear in 10s"
    );
