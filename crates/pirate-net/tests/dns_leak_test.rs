...
//!
//! These tests verify that DNS queries do NOT leak in cleartext.

use pirate_net::{DnsResolver, DnsConfig, DnsProvider};
...
async fn test_dns_provider_privacy_status() {
    let test_cases = vec![
        (DnsProvider::CloudflareDoH, true, "Cloudflare DoH should be private"),
        (DnsProvider::Quad9DoH, true, "Quad9 DoH should be private"),
        (DnsProvider::GoogleDoH, true, "Google DoH should be private"),
        (DnsProvider::System, false, "System DNS should NOT be private"),
    ];
...
fn test_dns_leak_prevention_checklist() {
    ...
    assert!(DnsProvider::CloudflareDoH.doh_url().unwrap().starts_with("https://"));
...
/// 
/// Run this with a packet capture tool to verify no cleartext DNS:
/// 
/// ```bash
...
fn test_custom_doh_endpoint() {
    let custom_provider = DnsProvider::CustomDoH("https://my-private-doh.example.com/dns-query".to_string());
