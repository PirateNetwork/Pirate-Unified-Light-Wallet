...
- `wallet restore <NAME> <MNEMONIC> [--birthday <HEIGHT>] [--mnemonic-language <LANG>]`
  - Output: wallet id string
- `wallet import-viewing <NAME> [--sapling-viewing-key <KEY>] [--ironwood-viewing-key <KEY>] --birthday <HEIGHT>`
  - Output: wallet id string
...
Change-address selection is automatic. Before Ironwood activation, Sapling-only
change returns to the legacy first selected Sapling spend address; after Ironwood
activation, Sapling-only change uses the wallet's internal Sapling change
address. Ironwood spends or outputs continue to use Ironwood internal change.
...
## `payment-disclosure` command group

Payment disclosures are Bech32-encoded single-output disclosure keys compatible
with Treasure Chest payment disclosure verification. They are generated by the
sender from the outgoing viewing key and transaction data. Sharing one
disclosure lets someone decrypt that specific Sapling output or Ironwood action;
it does not reveal the sender's wallet history or other outputs.

These commands accept `--wallet-id <WALLET_ID>`. If omitted, the active wallet is used.

- `payment-disclosure list [--wallet-id <WALLET_ID>] <TXID>`
  - Output: `PaymentDisclosure[]`
- `payment-disclosure sapling [--wallet-id <WALLET_ID>] <TXID> <OUTPUT_INDEX>`
  - Output: Sapling disclosure string
- `payment-disclosure ironwood [--wallet-id <WALLET_ID>] <TXID> <ACTION_INDEX>`
  - Output: Ironwood disclosure string
- `payment-disclosure verify [--wallet-id <WALLET_ID>] <DISCLOSURE>`
  - Output: `PaymentDisclosureVerification`

Direct JSON methods use the same names:
